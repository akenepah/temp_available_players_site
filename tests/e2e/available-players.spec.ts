...
test.describe("Available Players", () => {
  ...
  test("tiles the paper texture behind the entire page and shows the back-view hero artwork", async ({ page }) => {
    await serveFixture(page);
    await page.goto("/");
    ...
    const art = page.locator(".hero__players");
    await expect(art).toHaveAttribute("src", "/brand/hero-players-back.webp");
